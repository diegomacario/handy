...
public class CaptureManager2 : MonoBehaviour
{
    ...
    private CaptureTransform2[] m_Capturers = new CaptureTransform2[0];
    private string m_Filepath;

    List<float> m_Timestamps = new List<float>();
    ...
    // Gathers a reference to all `CaptureTransform2` instances we can find, which tracks
    // objects we want to capture transform information about
    private void GatherCapturers()
    {
        m_Capturers = GameObject.FindObjectsOfType<CaptureTransform2>();
    }
    ...
    private void HandleStopRecording()
    {
        ...
        // Reset the first capture flag
        foreach(CaptureTransform2 capturer in m_Capturers)
        {
